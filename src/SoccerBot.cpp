...
void SoccerBot::run() {
    ...
    while (!stopRequested && totalTime < 5.0) {
...
int SoccerBot::updateCameras(double dt) {
	...
	if (frontCamera != NULL && frontCamera->capturing()) {
		...
		if (image != NULL && image->dataYUYV != NULL && image->number != lastFrontFrameNr) {
			//s = Util::millitime();
	...
	if (rearCamera != NULL && rearCamera->capturing()) {
		...
		if (image != NULL && image->dataYUYV != NULL && image->number != lastRearFrameNr) {
